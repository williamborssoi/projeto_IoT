...
const char* ssid    = "oBorssoi";
const char* password = "w999428031";
IPAddress local_ip(192, 168, 50, 151);
IPAddress gateway(192, 168, 50, 1);
IPAddress subnet(255, 255, 255, 0);
WebServer server(80);

String montaHTML(bool statusLED, bool umidade = true);
// Se tiver um sensor DHT ou nenhum sensor descomente a linha 20
// Se tiver um sensor LM-35 comente a linha 20
#define __SENSOR_DHT__
#include <DHT.h>
#define DHTPIN 13
...
void mostraDados(float temperatura, int umidade, boolean mostraUmidade = true) {
  ...
  mostraTexto(30, 50, 0, "ADS Unoesc");
  display.display();
}
void medirTemperaturaUmidade() {
  float temperatura = 0;
  int umidade = 0;
#ifdef __SENSOR_DHT__
  ...
  mostraDados(temperatura, 0, false);
#endif
...
void manipula_sensor() {
  ...
  Serial.println("Enviando dados ao navegador em resposta à requisição Ajax");
...
void enviaHTML(bool statusLED) {
  ...
  digitalWrite(pinRELE, !statusLED);
#ifdef __SENSOR_DHT__
  server.send(200, "text/html", montaHTML(statusLED));
#else
  server.send(200, "text/html", montaHTML(statusLED, false));
#endif
}
...
String montaHTML(bool statusLED, bool umidade) {
  ...
  pagina += "let xhttp = new XMLHttpRequest();\n";
  pagina += "xhttp.onreadystatechange = function() {\n";
  pagina += "if (this.readyState == 4 && this.status == 200) {\n";
  pagina += "let jsonResponse = JSON.parse(this.responseText);\n";
  ...
  pagina += "<h2>Servidor Web ESP32</h1>\n";
  if (umidade) {
    pagina += "<h3>Sensor DHT</h3>\n";
  } else {
    pagina += "<h3>Sensor LM-35</h3>\n";
  }
